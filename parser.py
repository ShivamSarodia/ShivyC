...
class Parser:
    ...
    number of tokens consumed in that parse. If a match is not found and the
    `error` parameter is True, it raises a compiler exception; if `error` is
    False, it returns (None, 0).
