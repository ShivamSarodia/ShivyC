...
class Parser:
    ...
    def parse(self):
        ...
        if self.tokens[index:]:
            raise self.make_error("unexpected token", index, ParserError.AT)
        return node
    ...
    def expect_main(self, index):
        ...
        if match_start:
            ...
            return self.add_error(err, index, ParserError.AT)

        nodes = []
        while True:
            node, new_index = self.expect_statement(index)
            
            if not node:
                node, new_index = self.expect_declaration(index)
                if not node: break                
        ...
        if self.match_token(self.tokens[index:], token_kinds.close_brack):
            ...
            return self.add_error(err, index, ParserError.GOT)
        return (ast.MainNode(nodes), index)

    def expect_statement(self, index):
        node, new_index = self.expect_return(index)
        if node: return (node, new_index)

        node, new_index = self.expect_expr_statement(index)
        if node: return (node, new_index)
    ...
    def expect_semicolon(self, node, index):
        ...
        if self.match_token(self.tokens[index:], token_kinds.semicolon):
            ...
            return self.add_error(err, index, ParserError.AFTER)
    ...
    def expect_expression(self, index):
        ...
        if stack and isinstance(stack[0].item, ast.Node):
            ...
            return self.add_error("expected expression", index, ParserError.GOT)
    ...
    def add_error(self, message, index, message_type):
        ...
        """
        self.errors.append(
            (self.make_error(message, index, message_type),
             index))
        return (None, 0)
        
    def make_error(self, message, index, message_type):
        ...
        """
        return ParserError(message, index, self.tokens, message_type)
