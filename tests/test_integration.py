...
Tests the entire pipeline, from C source to executable. The tests in
IntegrationTests should focus on verifying that the generated ASM code behaves
as expected, and the tests in ErrorTests should focus on verifying that the
correct code generation errors are raised. If the main test point of a test is
to verify the lexer/parser/IL generation steps, put the test in the appropriate
test_*.py file.
